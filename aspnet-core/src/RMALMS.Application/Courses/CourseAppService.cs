...
namespace RMALMS.Courses
{
    ...
    public class CourseAppService : AsyncCrudAppService<Course, EditCourseDto, Guid, PagedResultRequestDto, CreateCourseDto, EditCourseDto>, ICourseAppService
    {
        ...
        {
            var now = DateTime.UtcNow;
            var commingSoonDate = now.AddDays(14);
            var userId = AbpSession.UserId.Value;

            // Check if the user is a system admin
            bool isSystemAdmin = _userService.UserHasRole(userId, StaticRoleNames.Tenants.Admin);

            // Get the roleCourseAdminId only if necessary
            int? roleCourseAdminId = null;
            if (!isSystemAdmin)
            {
                roleCourseAdminId = await _ws.GetAll<Role, int>()
                    .Where(r => r.Name == StaticRoleNames.Tenants.CourseAdmin)
                    .Select(r => r.Id)
                    .FirstOrDefaultAsync();
            }

            // Query base for CourseInstances and Courses
            var query = _ws.GetAll<CourseInstance>()
                .Where(ci => ci.Status == CourseSettingStatus.Active)
            ...
            if (!isSystemAdmin)
            {
                ...
                query = from ci in query
                        join uer in _ws.GetAll<UserExtraRole>()
                            .Where(u => u.EntityType == nameof(Course) && u.RoleId == roleCourseAdminId) on ci.Course.Id equals uer.EntityId into users
                        where ci.CreatorUserId == userId || users.Any(u => u.UserId == userId)
                        select ci;
            }

            // Common query with calculations and projections
            var courseDashboardQuery = query
                .Select(ci => new CourseDashboardDto
                {
                    Id = ci.Id,
                    StartDate = ci.StartTime,
                    EndDate = ci.EndTime,
                    Name = ci.Course.Name,
                    Description = ci.Course.Description,
                    RelationInfo = ci.Course.RelatedInformation,
                    CurrentPoint = ci.StartTime <= now && ci.EndTime >= now ? 100 : 0,
                    ComminSoonDatePoint = ci.StartTime <= commingSoonDate && ci.StartTime > now ? 10 : 0,
                    AlreadyStart = ci.StartTime <= now && ci.EndTime >= now,
                    IsCommingSoon = ci.StartTime <= commingSoonDate && ci.StartTime > now,
                    IsUpComming = ci.StartTime.HasValue && ci.StartTime.Value > commingSoonDate,
                    IsSelfPaced = !ci.NumberDayToStudy.HasValue,
                    ImageCover = ci.Course.ImageCover,
                    State = ci.Course.State,
                    IsOwner = ci.Course.CreatorUserId == userId,
                    IsArchived = (ci.EndTime ?? DateTime.MaxValue) < now
                })
                .OrderBy(s => s.CurrentPoint + s.ComminSoonDatePoint)
                .ThenByDescending(s => s.StartDate);

            return await courseDashboardQuery.ToListAsync();
        }
