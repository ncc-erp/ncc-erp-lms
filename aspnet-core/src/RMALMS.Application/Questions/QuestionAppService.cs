...
namespace RMALMS.Questions
{
    ...
    public class QuestionAppService : AsyncCrudAppService<Question, QuestionDto, Guid, PagedResultRequestDto, CreateQuestionDto, QuestionDto>, IQuestionAppService
    {
        ...
        {
            // Early return if quizId is empty
            ...
            var query =
                from qmq in _ws.GetAll<QuestionQuiz>().Where(qmq => qmq.QuizId == quizId)
                join q in _ws.GetAll<Question>() on qmq.QuestionId equals q.Id
                join a in _ws.GetAll<Answer>() on q.Id equals a.QuestionId into answers
                select new QuestionDto
                {
                    QuestionQuizId = qmq.Id,
                    ...
                    Group = q.Group,
                    Index = qmq.Index,
                    Answers = answers.Select(s => new AnswerDto
                    {
                        Id = s.Id,
                        IsCorrect = s.IsCorrect,
                        LAnswer = s.LAnswer,
            ...
            // Order by Index and return as a list
            return await query.OrderBy(s => s.Index).ToListAsync();
        }
